                    ...
                    <div class="card-body p-md-5">
                        <div class="row justify-content-center mb-4">
                            <div class="col-md-10 col-lg-6 col-xl-5 order-2 order-lg-1">
                                ...
                                <?php endif; ?>
                                <form class="mx-1 mx-md-4 mt-4" action="/register" method="post">
                                    <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
                                    <div class="mb-4">
                                        <label class="form-label" for="customer_name">Họ và tên</label>
                                        <input type="text" name="customer_name" value="<?= old('customer_name') ?>" id="customer_name" class="form-control" placeholder="Nhập họ và tên của bạn" />
                                    </div>
                                    <div class="mb-4">
                                        <label class="form-label" for="customer_email">Email của bạn</label>
                                        <input type="email" name="customer_email" value="<?= old('customer_email') ?>" id="customer_email" class="form-control" placeholder="Nhập địa chỉ email của bạn" />
                                    </div>
                                    <div class="mb-4">
                                        <label class="form-label" for="customer_password">Mật khẩu</label>
                                        <input type="password" name="customer_password" value="<?= old('customer_password') ?>" id="customer_password" class="form-control" placeholder="Nhập mật khẩu" />
                                    </div>
                                    <div class="mb-4">
